...
const middlewareAPI = (req, res, next) => {
    ...
    if (x_id !== config.X_CLIENT_ID || x_secret !== config.X_CLIENT_SECRET){
        console.error(`${timestamp} error 401: Unauthorized| {user_side} x-id: ${x_id} x-secret ${x_secret} | {server_side} x-id: ${config.X_CLIENT_ID} x-secret ${config.X_CLIENT_SECRET}`)
    ...
    if (reference.length != 40 || reference == null || configuration['credit'] <= 0){
        console.error(`${timestamp} error 401: Unauthorized | reference len ${reference.length} | credit ${configuration['credit']}`)
        return res.status(401).json({
            code : 401,
            errors : "Unauthorized"
        });
    ...
    if (game_id != GAME_ID){
        console.error(`${timestamp} error 404: Not found game | game-id ${game_id}`)
